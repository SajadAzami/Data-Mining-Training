...
# 4. Learning * Model from Data
print('\nStochastic Gradient Descent Model:')
sg_model = SGDRegressor()
sg_line = sg_model.fit(train_X, train_Y)
sg_predictions = sg_model.predict(test_X)
title = 'Stochastic Gradient Descent'
RSS = sum((test_Y - sg_predictions) ** 2)
print('Stochastic Gradient Descent\nTest RSS: ' + str(RSS))
cv_risk = math.sqrt(sum(abs(cross_val_score(sg_model,
                                            train_X, train_Y, scoring='mean_squared_error', cv=10))) / 10)
print('10-fold CV with RMSE: ' + str(cv_risk))
plt.plot(np.linspace(0, 40, 40), sg_predictions, 'r', label="predictions")
plt.plot(np.linspace(0, 40, 40), test_Y, label="real values")
plt.legend(loc='lower right')
plt.title(title)
plt.show()
